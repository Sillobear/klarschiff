...
def EMA(df, n, ID):  
    EMA = df[ID].ewm(min_periods=n,span=n).mean()  
    df = df.join(EMA.rename('EMA' + str(n)))  
    return df
...
def ATR(df, n):  
    ...
    sATR = TR_s.ewm(span = n, min_periods = n,adjust=False).mean()  
    df = df.join(sATR.rename('ATR'+str(n)))   
    return df
...
def Zscore_rolling(df,key,window):
    roll = df[key].rolling(window)
    df['z-score'] = (df[key] - roll.mean()) / roll.std()
    return df

#MACD, MACD Signal and MACD difference  
#MIS Juli 2022
def MACD(df, n_fast, n_slow, n, key):  
    EMAfast = df['Close'].ewm(span=n_fast,adjust=False).mean() 
    EMAslow = df['Close'].ewm(span=n_slow,adjust=False).mean() 
    MACD = EMAfast - EMAslow
    MACDsign = MACD.ewm(span=n).mean()
    MACDdiff = (MACD - MACDsign)
    df["MACD"+key]=MACD
    df["MACDsign"+key]=MACDsign
    df["MACDdif"+key]=MACDdiff
    #print(df.tail(20))
    return df
